...
const Modal = ({sourceName, setShowModal}) => {
    ...
    const downloadImage = async () => {
        // const response = await fetch(`http://127.0.0.9:8080/steganographer/image?name=${sourceName}`);
        const response = await fetch(`https://steganographer-flask-server.onrender.com/steganographer/image?name=${sourceName}`);
    
        const blobImage = await response.blob();
        // const blobOb = blobImage.slice(0, blobImage.size, "image/png")
    
        // const href = URL.createObjectURL(blobOb);
